...
int main()
{
	...
	{
		(*(t_message*)addr).join_flag[2] = 1; // A join
	}
	//gamestart
	system("clear");
	printf("game start\n\n");
	...
	while(round <3)
	{
		if((*(t_message*)addr).b[3] == 0 && (*(t_message*)addr).a[3] == 0)
		{
	...
	if((*(t_message*)addr).c[3] >= (*(t_message*)addr).b[3] && (*(t_message*)addr).c[3] >= (*(t_message*)addr).a[3])
	{
		printf("you win\n");
	}
	else
...
void *betting_function(void *data)
{
	...
	while(i<3)
	{
		...
		printf("horse %d :", i+1);
		scanf("%d", &input);
		if(input > (*(t_message*)addr).c[3])
		{
			printf("you can't betting\n");
			continue;
		}
		(*(t_message*)addr).c[i] = input;
		(*(t_message*)addr).c[3] = (*(t_message*)addr).c[3] - input;
		i = i + 1;
	}
	while(1)
	{
